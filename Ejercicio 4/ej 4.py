...
class Calculadora(object):
    ...
    def __init__(self):
        ...
        ttk.Button(mainframe, text='+', command=partial(self.ponerOPERADOR, '+')).grid(column=1, row=6, sticky=W)
        ...
        ttk.Button(mainframe, text='AC', command=self.borrarPanel).grid(column=3, row=8, sticky=W)
        self.__panel.set('0')
        panelEntry.focus()
    ...
    def ponerNUMERO(self, numero):
        if self.__operadorAux==None:
            ...
            if "/" in self.__panel.get():
                ...
                print(self.__primerOperando)
                self.__panel.set(numero)
            else:
                self.__operadorAux=None
                valor=self.__panel.get()
                self.__primerOperando=Fraccion(int(valor),1)
                self.__panel.set(numero) 
            # else:
            #     self.__operadorAux=None
            #     valor=self.__panel.get()
    ...
    def ponerOPERADOR(self, op):
        if op=='=':
            if "/" in self.__panel.get():
                ...
                operacion=self.__operador.get()
                operando=int(self.__panel.get())
                print(operando)
                self.__segundoOperando=Fraccion(operando,1)
                self.resolverOperacion(self.__primerOperando, operacion, self.__segundoOperando)
                print(self.resolverOperacion)
                self.__operador.set('')
                self.__operadorAux=None
